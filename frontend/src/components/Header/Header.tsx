  ...
  const handleSearch = async (term: string) => {
    ...
    try {
      const response = await axios.get(
        `https://fe1111.projects.academy.onlyjs.com/api/v1/products?${term}`,
        {
          params: {
            limit: 85,
            ofset: 0,
      ...
      console.log(response.data.data); // array dönüyo
      const results = response.data.data.results || []; // `results` varsa al, yoksa boş dizi kullan
